...
def config_display_mask():
    ...
    global display, maxpattern, qubits_needed, UseHex, UseQ16, UseTee
    if qubits_needed > 16:
        display = ibm_q32x
        maxpattern = '0' * 32
...
def find_qasm_file(qasmfileinput):
    """Find and validate QASM file path"""
    global scriptfolder
    qasm_dir = os.environ.get("QASM_DIR", "/app/files/qasm")

    if not qasmfileinput or qasmfileinput == '':
        ...
        qasmfilename = 'expt12.qasm'
    else:
        qasmfilename = qasmfileinput

    print("QASM File: ", qasmfilename)

    # Try with QASM_DIR environment variable first
    if '/' not in qasmfilename:
        qasmfilename = os.path.join(qasm_dir, qasmfilename)

    if not os.path.isfile(qasmfilename):
        # Fallback to scriptfolder
        qasmfilename = os.path.join(scriptfolder, qasmfileinput if qasmfileinput else 'expt.qasm')

    if not os.path.isfile(qasmfilename):
        # Final fallback
        qasmfilename = os.path.join(scriptfolder, 'expt.qasm')
...
def execute_circuit_once(qcirc, qasm_circuit_obj):
    ...
    try:
        ...
        try:
            ...
            print('Unable to render quantum circuit drawing')
        ...
        try:
            ...
            if UseLocal:
                sleep(3)
            thinking = False  # Signal display thread
...
while outer_control_loop:
    ...
    if cmd.get("command") == "run":
        ...
        # Find and validate the QASM file
    ...
    # Configure display mask based on circuit width
    ...
    # Check if loop mode is enabled via config
    loop_iterations = 1  # Default: single execution
    try:
        _config_path = Path(os.environ.get("CONTROL_DIR", "/app/files/control")) / "config.json"
        if _config_path.exists():
            with open(_config_path, 'r') as _f:
                _config = json.load(_f)
                if _config.get("loop_mode"):
                    loop_iterations = _config.get("loop_iterations", 10)
                    print(f"[CONTROL] Loop mode enabled, running {loop_iterations} iterations")
    except Exception as _e:
        print(f"[CONTROL] Could not read loop mode config: {_e}")

    # Execute circuit(s) - NEW ARCHITECTURE REPLACES while Looping:
    for iteration in range(loop_iterations):
        if loop_iterations > 1:
            print(f"\n[LOOP] Iteration {iteration + 1}/{loop_iterations}")

        result = execute_circuit_once(qcirc, qcirc)

        if loop_iterations > 1 and iteration < loop_iterations - 1:
